export default {
  nav: {
    dashboard: 'Accueil',
    transactions: 'Transactions',
    budgets: 'Budgets',
    accounts: 'Comptes',
    settings: 'Paramètres'
  },

  common: {
    save: 'Enregistrer',
    cancel: 'Annuler',
    delete: 'Supprimer',
    edit: 'Modifier',
    add: 'Ajouter',
    close: 'Fermer',
    confirm: 'Confirmer',
    back: 'Retour',
    seeAll: 'Voir tout',
    filters: 'Filtres',
    reset: 'Réinitialiser',
    loading: 'Chargement…',
    noData: 'Aucune donnée',
  },

  dashboard: {
    title: 'Tableau de bord',
    totalBalance: 'Solde total',
    period: 'Période',
    upcoming: 'À venir',
    recentTransactions: 'Transactions récentes',
    monthlyOverview: 'Aperçu',
    balanceByAccount: 'Balance par compte',
    expenseBreakdown: 'Répartition des dépenses',
    monthlyFlow: 'Flux du mois',
    cumulativeBalance: 'Balance cumulée',
    help: {
      hero: {
        title: 'Solde & Période',
        body: 'Le solde total reflète l\'état réel de vos comptes à aujourd\'hui — transactions passées uniquement. La période sélectionnée en bas détermine la plage analysée par les KPIs, les graphiques et les budgets. Naviguez avec les flèches, ou appuyez sur le mois pour revenir à aujourd\'hui.',
      },
      kpi: {
        title: 'Aperçu de la période',
        body: 'Revenus et Dépenses comptabilisent les transactions dont la date est passée ou égale à aujourd\'hui sur la période choisie. Les virements entre comptes sont exclus. À venir regroupe les dépenses planifiées dans le futur — elles n\'impactent pas encore les totaux.',
      },
      balanceByAccount: {
        title: 'Balance par compte',
        body: 'Solde de chaque compte à la fin de la période sélectionnée. Pour le mois en cours, il reflète l\'état à aujourd\'hui.',
      },
      expenseBreakdown: {
        title: 'Répartition des dépenses',
        body: 'Part de chaque budget dans les dépenses de la période sélectionnée. Seules les transactions passées rattachées à un budget sont comptabilisées.',
      },
      monthlyFlow: {
        title: 'Flux de la période',
        body: 'Courbe cumulative des flux nets (revenus moins dépenses) jour par jour sur la période sélectionnée. Seules les transactions passées sont prises en compte.',
      },
    },
  },

  transactions: {
    title: 'Transactions',
    new: 'Nouvelle transaction',
    edit: 'Modifier la transaction',
    income: 'Revenus',
    expense: 'Dépenses',
    recurring: 'Récurrentes',
    all: 'Toutes',
    label: 'Libellé',
    amount: 'Montant',
    account: 'Compte',
    budget: 'Budget',
    date: 'Date',
    location: 'Lieu',
    category: 'Catégorie',
    search: 'Rechercher…',
    dateRange: {
      all:       'Tout',
      thisMonth: 'Ce mois',
      lastMonth: 'Mois dernier',
      thisYear:  'Cette année',
    },
    color: 'Couleur',
    icon: 'Icône',
    type: 'Type',
    period: 'Période',
    isRecurring: 'Transaction récurrente',
    recurringUnit: {
      day: 'Jour',
      week: 'Semaine',
      month: 'Mois',
      year: 'Année',
    },
    transfer: 'Virement',
    transferFrom: 'Compte source',
    transferTo: 'Compte destinataire',
    noCurrencyMatch: 'Aucun compte avec la même devise.',
    noAccount: 'Créez d\'abord un compte pour ajouter une transaction.',
    empty: 'Aucune transaction',
    emptyFiltered: 'Aucun résultat pour ce filtre',
    deleteConfirm: 'Supprimer cette transaction ?',
    deleteConfirmMessage: 'Cette action est irréversible.',
  },

  budgets: {
    title: 'Budgets',
    new: 'Nouveau budget',
    edit: 'Modifier le budget',
    label: 'Nom du budget',
    monthlyCap: 'Plafond mensuel',
    spent: 'Dépensé',
    remaining: 'Restant',
    over: 'Dépassé',
    empty: 'Aucun budget',
    deleteConfirm: 'Supprimer ce budget ?',
    deleteConfirmMessage: 'Les transactions liées ne seront pas supprimées. Cette action est irréversible.',
  },

  accounts: {
    title: 'Comptes',
    new: 'Nouveau compte',
    edit: 'Modifier le compte',
    label: 'Nom du compte',
    balance: 'Solde',
    initialBalance: 'Solde initial',
    currency: 'Devise',
    type: {
      label: 'Type',
      checking: 'Courant',
      savings: 'Épargne',
      investment: 'Investissement',
    },
    empty: 'Aucun compte',
    deleteConfirm: 'Supprimer ce compte ?',
    deleteConfirmMessage: 'Toutes les transactions associées seront également supprimées. Cette action est irréversible.',
  },

  settings: {
    title: 'Paramètres',
    preferences: {
      title: 'Apparence',
      description: 'Changer le thème et les couleurs de l\'application',
      theme: 'Thème',
      dark: 'Sombre',
      light: 'Clair',
      accent: 'Couleur principale',
    },
    data: {
      data: 'Données',
      description: 'Exportez vos données dans un fichier JSON pour les sauvegarder ou les transférer. Importez un fichier pour fusionner son contenu avec votre base locale.',
      exportButton: 'Exporter mes données',
      exportAction: 'Exporter',
      importButton: 'Importer mes données',
      importAction: 'Importer',
      importConfirm: 'Importer cette sauvegarde ?',
      importConfirmMessage: 'Les enregistrements existants avec le même identifiant seront écrasés. Les nouveaux enregistrements seront ajoutés.',
      importSuccess: 'Import réussi',
      importError: 'Échec de l\'import : {msg}',
      resetButton: 'Supprimer toutes mes données',
      resetAction: 'Supprimer',
      resetConfirm: 'Supprimer toutes les données ?',
      resetConfirmMessage: 'Cela supprimera définitivement tous vos comptes, budgets et transactions. Cette action est irréversible.',
      resetSuccess: 'Toutes les données ont été supprimées',
    },
    sync: {
      title: 'Synchronisation',
      description: 'Ouvrez cette page sur les deux appareils. Partagez votre code avec l\'autre, ou entrez son code pour vous connecter.',
      myDevice: 'Mon appareil',
      myDevicePlaceholder: 'Ex : iPhone de Pierre',
      myDeviceRequired: 'Nommez votre appareil pour pouvoir synchroniser.',
      searching: 'En recherche…',
      connected: '1 appareil connecté | {count} appareils connectés',
      myCode: 'Mon code',
      copy: 'Copier',
      copied: 'Copié !',
      otherDevice: 'Autre appareil',
      codePlaceholder: 'mot1-mot2-mot3-mot4',
      connect: 'Se connecter',
      connectedTo: 'Connecté à {name}',
      disconnect: 'Déconnecter',
      peersSection: 'Appareils connectés',
      recents: 'Récents',
      rename: 'Renommer',
      forget: 'Oublier',
      syncButton: 'Synchroniser maintenant',
      syncing: 'Synchronisation…',
      success: 'Synchronisé avec succès',
      error: 'Erreur : {msg}',
    },
    advanced: {
      title: 'Avancé',
      turn: {
        title: 'Serveur TURN',
        description: 'Sur certains appareils ou configurations réseau, la connexion directe entre deux appareils peut échouer. Un serveur TURN sert de relai chiffré pour garantir la connexion.',
        link: 'Comment configurer votre propre serveur TURN ?',
        noServer: 'Aucun serveur configuré',
        host: 'Hôte',
        port: 'Port',
        username: 'Identifiant',
        password: 'Mot de passe',
        save: 'Enregistrer',
        saved: '✓ Enregistré',
        reset: 'Réinitialiser',
      },
    },
  },

  pwa: {
    update: {
      message: 'Une nouvelle version est disponible.',
      later: 'Plus tard',
      refresh: 'Mettre à jour',
    },
    install: {
      title: 'Installer Tunasse',
      description: 'Accédez à l\'app depuis votre écran d\'accueil, même hors connexion.',
      ios: 'Appuyez sur ⎙ Partager puis « Sur l\'écran d\'accueil ».',
      install: 'Installer',
    },
  },
  footer: {
    sourceCode: 'Code source',
  },
}
