export default {
  nav: {
    dashboard: 'Home',
    transactions: 'Transactions',
    budgets: 'Budgets',
    accounts: 'Accounts',
    settings: 'Settings'
  },

  common: {
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    add: 'Add',
    close: 'Close',
    confirm: 'Confirm',
    back: 'Back',
    seeAll: 'See all',
    filters: 'Filters',
    reset: 'Reset',
    loading: 'Loading…',
    noData: 'No data',
  },

  dashboard: {
    title: 'Dashboard',
    totalBalance: 'Total balance',
    period: 'Period',
    upcoming: 'Upcoming',
    recentTransactions: 'Recent transactions',
    monthlyOverview: 'Overview',
    balanceByAccount: 'Balance by account',
    expenseBreakdown: 'Expense breakdown',
    monthlyFlow: 'Monthly flow',
    cumulativeBalance: 'Cumulative balance',
    help: {
      hero: {
        title: 'Balance & Period',
        body: 'Total balance reflects the real state of your accounts as of today — past transactions only. The selected period below determines the range analysed by KPIs, charts and budgets. Use the arrows to navigate, or tap the month to jump back to today.',
      },
      kpi: {
        title: 'Period overview',
        body: 'Income and Expenses count transactions dated up to today within the selected period. Transfers between accounts are excluded. Upcoming groups future-dated expenses — they don\'t affect the totals yet.',
      },
      balanceByAccount: {
        title: 'Balance by account',
        body: 'Balance of each account at the end of the selected period. For the current month, it reflects today\'s state.',
      },
      expenseBreakdown: {
        title: 'Expense breakdown',
        body: 'Share of each budget in the expenses of the selected period. Only past transactions linked to a budget are counted.',
      },
      monthlyFlow: {
        title: 'Period flow',
        body: 'Cumulative net flow (income minus expenses) day by day over the selected period. Only past transactions are included.',
      },
    },
  },

  transactions: {
    title: 'Transactions',
    new: 'New transaction',
    edit: 'Edit transaction',
    income: 'Income',
    expense: 'Expenses',
    recurring: 'Recurring',
    all: 'All',
    label: 'Label',
    amount: 'Amount',
    account: 'Account',
    budget: 'Budget',
    date: 'Date',
    location: 'Location',
    category: 'Category',
    type: 'Type',
    period: 'Period',
    isRecurring: 'Recurring transaction',
    recurringUnit: {
      day: 'Day',
      week: 'Week',
      month: 'Month',
      year: 'Year',
    },
    search: 'Search…',
    dateRange: {
      all:       'All',
      thisMonth: 'This month',
      lastMonth: 'Last month',
      thisYear:  'This year',
    },
    icon:  'Icon',
    color: 'Color',
    transfer: 'Transfer',
    transferFrom: 'From account',
    transferTo: 'To account',
    noCurrencyMatch: 'No account with the same currency.',
    noAccount: 'Create an account first to add a transaction.',
    empty: 'No transactions',
    emptyFiltered: 'No results for this filter',
    deleteConfirm: 'Delete this transaction?',
    deleteConfirmMessage: 'This action cannot be undone.',
  },

  budgets: {
    title: 'Budgets',
    new: 'New budget',
    edit: 'Edit budget',
    label: 'Budget name',
    monthlyCap: 'Monthly cap',
    spent: 'Spent',
    remaining: 'Remaining',
    over: 'Over budget',
    empty: 'No budgets',
    deleteConfirm: 'Delete this budget?',
    deleteConfirmMessage: 'Linked transactions will not be deleted. This action cannot be undone.',
  },

  accounts: {
    title: 'Accounts',
    new: 'New account',
    edit: 'Edit account',
    label: 'Account name',
    balance: 'Balance',
    initialBalance: 'Initial balance',
    currency: 'Currency',
    type: {
      label: 'Type',
      checking: 'Checking',
      savings: 'Savings',
      investment: 'Investment',
    },
    empty: 'No accounts',
    deleteConfirm: 'Delete this account?',
    deleteConfirmMessage: 'All associated transactions will also be deleted. This action cannot be undone.',
  },

  settings: {
    title: 'Settings',
    preferences: {
      title: 'Appearance',
      description: 'Change theme and colors of the application',
      theme: 'Theme',
      dark: 'Dark',
      light: 'Light',
      accent: 'Accent color',
    },
    data: {
      data: 'Data',
      description: 'Export your data to a JSON file to back it up or transfer it. Import a file to merge its contents with your local database.',
      exportButton: 'Export backup',
      exportAction: 'Export',
      importButton: 'Import backup',
      importAction: 'Import',
      importConfirm: 'Import this backup?',
      importConfirmMessage: 'Existing records with the same ID will be overwritten. New records will be added.',
      importSuccess: 'Import successful',
      importError: 'Import failed: {msg}',
      resetButton: 'Delete all my data',
      resetAction: 'Delete',
      resetConfirm: 'Delete all data?',
      resetConfirmMessage: 'This will permanently delete all your accounts, budgets and transactions. This action cannot be undone.',
      resetSuccess: 'All data has been deleted',
    },
    sync: {
      title: 'Synchronization',
      description: 'Open this page on both devices. Share your code with the other device, or enter its code to connect.',
      myDevice: 'My device',
      myDevicePlaceholder: 'E.g. Pierre\'s iPhone',
      myDeviceRequired: 'Name your device before syncing.',
      searching: 'Searching…',
      connected: '1 device connected | {count} devices connected',
      myCode: 'My code',
      copy: 'Copy',
      copied: 'Copied!',
      otherDevice: 'Other device',
      codePlaceholder: 'word1-word2-word3-word4',
      connect: 'Connect',
      connectedTo: 'Connected to {name}',
      disconnect: 'Disconnect',
      peersSection: 'Connected devices',
      recents: 'Recents',
      rename: 'Rename',
      forget: 'Forget',
      syncButton: 'Sync now',
      syncing: 'Syncing…',
      success: 'Synced successfully',
      error: 'Error: {msg}',
    },
    advanced: {
      title: 'Advanced',
      turn: {
        title: 'TURN Server',
        description: 'On some devices or network configurations, a direct connection between two devices may fail. A TURN server acts as an encrypted relay to guarantee the connection.',
        link: 'How to set up your own TURN server ?',
        noServer: 'No server configured',
        host: 'Host',
        port: 'Port',
        username: 'Username',
        password: 'Password',
        save: 'Save',
        saved: '✓ Saved',
        reset: 'Reset',
      },
    },
  },

  pwa: {
    update: {
      message: 'A new version is available.',
      later: 'Later',
      refresh: 'Update',
    },
    install: {
      title: 'Install Tunasse',
      description: 'Access the app from your home screen, even offline.',
      ios: 'Tap ⎙ Share then "Add to Home Screen".',
      install: 'Install',
    },
  },
  footer: {
    sourceCode: 'Source code',
  },
}
